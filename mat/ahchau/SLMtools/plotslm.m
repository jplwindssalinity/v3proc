function hfig = plotslm(slm,varargin)
...
% Release date: 2/6/07
% 
% Modified by A.Chau 2/15/12

p=inputParser;
p.addParamValue('fignum',[],@isnumeric);
p.addParamValue('scatter','no',@ischar);
p.addParamValue('LineStyle','-');
p.addParamValue('LineColor','r');
p.addParamValue('LineWidth',1.5);
p.addParamValue('DataMarker','o');
p.addParamValue('DataColor','b');
p.addParamValue('DataSize',3);
p.addParamValue('KnotStyle','--');
p.addParamValue('KnotColor','g');
p.addParamValue('Grid','off');
p.parse(varargin{:});
...
% initial plot parameters
params.LineStyle = p.Results.LineStyle;
params.LineColor = p.Results.LineColor;
params.LineWidth = p.Results.LineWidth;

params.DataMarker = p.Results.DataMarker;
params.DataColor = p.Results.DataColor;
params.DataSize = p.Results.DataSize;

params.KnotStyle = p.Results.KnotStyle;
params.KnotColor = p.Results.KnotColor;

params.Grid = p.Results.Grid;
  ...
  if isfield(params.model,'x') && ~isempty(params.model.x)
    ...
    switch p.Results.scatter
        ...
        case 'size'
            ...
            h = scatter(params.model.x,params.model.y, ...
                (params.model.prescription.Weights+eps) ...
                /max(params.model.prescription.Weights)*10*params.DataSize, ...
                params.DataMarker);
            
            colind = strfind('rgbcmyk',params.DataColor);
            allcols =          [0         0    1.0000;
                0    0.5000         0;
                1.0000         0         0;
                0    0.7500    0.7500;
            ...
            %this is get(0,'DefaultAxesColorOrder')
            
            set(h,'CData',allcols(colind,:));
            
        case 'color'
            h = scatter(params.model.x,params.model.y, ...
                ones(size(params.model.x))*params.DataSize*10, ...
                params.model.prescription.Weights, ...
                params.DataMarker);
