...
% by Steve Hoelzer
% 2009-03-25
% mysubfigure edited by A. Chau to take in an existing figure handle
...
if ~isscalar(m) || ~isscalar(n)
    error('Grid dimensions must be scalar values.')
end
if m < 0 || n < 0
...
pos = [le, bo-18, ri-le, to-bo]; % [left, bottom, width, height]

% Display figure

if ~exist('hh','var')
    hh = figure('Position',pos);
else
    set(figure(hh),'Position',pos);
end
