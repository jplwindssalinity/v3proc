...
% function h=hline(y, linetype, label,varargin)
% 
...
% p.addParamValue('labellocation','left') % can also input 'right'

p = inputParser;
p.addParamValue('labellocation','left')
p.parse(varargin{:});
...
if length(y)>1  % vector input
    for I=1:length(y)
        ...
        h(I)=myhline(y(I),linetype,label, ...
    ...
    if ~isempty(label)
        ...
        if yunit<0.2
            switch p.Results.labellocation
                case 'left'
                    text(x(1)+0.02*(x(2)-x(1)),y+0.02*yrange,label, ...
                        'color',get(h,'color'))
                case 'right'
                    text(x(end)-0.02*(x(2)-x(1)),y+0.02*yrange,label, ...
                        'color',get(h,'color'), ...
