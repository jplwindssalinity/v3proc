    ...
    MeasSpot*    meas_spot)
{
	...
	for (int slice_idx = 0; slice_idx < total_slices; slice_idx++)
	{
		...
		float f1, bw, f2;
		if (! qscat->ses.GetSliceFreqBw(slice_idx, &f1, &bw))
			return(0);
		f2 = f1 + bw;
		...
		if (! FindSlice(&antenna_frame_to_gc, spacecraft, qscat, look, azim,
    ...
    int          max_slices)
{
	...
	if (! GetPeakSpatialResponse2(&antenna_frame_to_gc, spacecraft, beam,
		qscat->sas.antenna.spinRate, &center_look, &center_azim))
	{
		return(0);
	}
        ...
        TargetInfo(&antenna_frame_to_gc, spacecraft, qscat, center_vector,
            &tip);
	...
	if (! FindPeakResponseAtFreq(&antenna_frame_to_gc, spacecraft, qscat,
            ...
            &pos_azim, &dummy_gain) ||
	    ! FindPeakResponseAtFreq(&antenna_frame_to_gc, spacecraft, qscat,
    ...
    float        contour_level)
{
	...
	float gp_max;
    if (beam->GetSpatialResponse(look, azim, tip.roundTripTime,
    ...
    EarthPosition*     centroid)
{
	...
	f_look[0] = look;
	f_azim[0] = azim;
	if (! FindPeakResponseAtFreq(antenna_frame_to_gc, spacecraft, qscat, freq_1,
	...
	f_look[1] = look;
	f_azim[1] = azim;
	if (! FindPeakResponseAtFreq(antenna_frame_to_gc, spacecraft, qscat, freq_2,
	...
	float peak_gain;
	if (! FindPeakResponseForSlice(antenna_frame_to_gc, spacecraft, qscat, f_look,
	...
	{
		if (! FindSliceCorners(antenna_frame_to_gc, spacecraft, qscat,
	...
	else
	{
		...
		if (! FindResponseBetween(f_look, f_azim, f_gain, target_gain,
			&tc_look, &tc_azim))
		{
    ...
    int                ignore_range)
{
	...
	do
	{
		...
		if (! FindFreq(antenna_frame_to_gc, spacecraft, qscat, target_freq,
            freq_tol, look, azim))
		{
			return(0);
		}
		...
		if (! FreqGradient(antenna_frame_to_gc, spacecraft, qscat, *look,
    ...
    double*            df_dazim)
{
	...
	if (! TargetInfo(antenna_frame_to_gc, spacecraft, qscat, vector_look_1,
            &tip_1) ||
		! TargetInfo(antenna_frame_to_gc, spacecraft, qscat, vector_look_2,
            &tip_2))
	{
		return(0);
	}
	...
	*df_dlook = (tip_2.basebandFreq - tip_1.basebandFreq) / (look_2 - look_1);
	...
	if (! TargetInfo(antenna_frame_to_gc, spacecraft, qscat, vector_azim_1,
            &tip_1) ||
		! TargetInfo(antenna_frame_to_gc, spacecraft, qscat, vector_azim_2,
            &tip_2))
	{
		return(0);
	}
    ...
    int                ignore_range)
{
	...
	else if (mid_response >= response[0] && mid_response >= response[1])
	{
		...
		double s[3], c[3];
		if (! QuadFit(antenna_frame_to_gc, spacecraft, qscat, look_array,
            azim_array, s, c))
		{
			return(0);
		}
		if (! PeakFit(c, peak_response))
			return(0);
...
// FindResponseBetween //
//---------------------//
// Finds the target response along the line connecting the provided look and
// azimuth points
	...
	double*				tc_azim)
{
	...
	float delta_response = f_response[1] - f_response[0];

	float response_frac = (target_response - f_response[0]) / delta_response;
	*tc_look = f_look[0] + response_frac * delta_look;
	*tc_azim = f_azim[0] + response_frac * delta_azim;
	return(1);
}
    ...
    double             corner_azim[2])
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		if (! QuadFit(antenna_frame_to_gc, spacecraft, qscat,
...
//
// This function locates the maximum spatial response of a beam in the antenna frame,
