...
FbbTable::Deallocate()
{
    free_array((void*)a, 3, BYU_NUM_BEAMS, BYU_ORBIT_POSITION_BINS,
        BYU_AZIMUTH_BINS);
    free_array((void*)b, 3, BYU_NUM_BEAMS, BYU_ORBIT_POSITION_BINS,
        BYU_AZIMUTH_BINS);
    free_array((void*)c, 3, BYU_NUM_BEAMS, BYU_ORBIT_POSITION_BINS,
        BYU_AZIMUTH_BINS);
    free_array((void*)d, 3, BYU_NUM_BEAMS, BYU_ORBIT_POSITION_BINS,
        BYU_AZIMUTH_BINS);
    return(1);
}
    ...
    for (int bm = 0; bm < 2; bm++)
    {
        ...
        for (int o = 0; o < BYU_ORBIT_POSITION_BINS; o++)
        {
            ...
            for (int ah = 0; ah < BYU_AZIMUTH_BINS; ah++)
            {
                ...
                {
                    fprintf(stderr,
                        "FbbTable::read error sanity check failed.\n");
                    return(0);
                }
    ...
    float    azimuth_angle)
{
    ...
    float retval = ct1 * ca1 * table[t1][a1] +
        ct1 * ca2 * table[t1][a2] +
        ct2 * ca1 * table[t2][a1] +
        ct2 * ca2 * table[t2][a2];
    return(retval);
