    ...
    L2AFrame*  frameGroup50)
{
  ...
  while (frameGroup25[2*off].ati < frameGroup25[0].ati+1)
    ...
    //  (if current 25km cti is less than start,
    // drop current cases until = start.cti)
...
L2A::L2A()
:   _status(OK), _headerRead(0), _headerWritten(0), idx(0)
{
    return;
}
