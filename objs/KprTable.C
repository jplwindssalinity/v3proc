...
static const char rcs_id_kprtable_c[] =
	"@(#) $Id$";
...
KprTable::KprTable(int number_of_beams, float slice_bandwidth,
		   int slices_per_spot, 
...
int 
KprTable::Accumulate(MeasSpotList* quiet, MeasSpotList* noisy)
{
  ...
  while(noisy_spot && quiet_spot){
	...
	fprintf(stderr,"KprTable::Accumulate-- Too many slices.\n");
	return(0);
      }
      //------------------------------------------------------//
    ...
    if(slice_number < _slicesPerSpot){   
      fprintf(stderr,"KprTable::Accumulate-- Too few slices.\n");
      return(0);
    }

    if(noisy_slice || quiet_slice){
      fprintf(stderr,"KprTable::Accumulate-- SlicesPerSpot don't match\n");
      return(0);
    }  
  ...
  if(noisy_spot || quiet_spot){
    fprintf(stderr,"KprTable::Accumulate-- SpotsPerFrame don't match\n");
    return(0);
  }
...
int
KprTable::Accumulate(Meas* quiet, Meas* noisy, int beam_idx, int slice_idx,
		     double azimuth){
  float azi, sqrdif;
  int azi_idx;
...
//**********************************************
int
KprTable::Accumulate(L00Frame* quiet , L00Frame* noisy){
  int slice_number=0;
...
int KprTable::Smooth(int filter_size){
  if (filter_size % 2 != 1){
    fprintf(stderr,"KprTable::Smooth: filter_size must be an odd number\n");
    return(0);
  }
...
//-------------------------------//
//  KprTable::Interpolate        //
//-------------------------------//
float KprTable::Interpolate(int beam_number, int slice_number, float azimuth){
  float azi, kpr;
  int az1, az2;
  int slice_idx;
  ...
  if(slice_idx < 0 || slice_idx >= _slicesPerSpot){
    fprintf(stderr,"Error KprTable::Interpolate: Bad slice number\n");
    exit(1);
  }
...
//--------------------------------//
//  KprTable::NormalizeFrom3Sigma //
//--------------------------------//   
int KprTable::NormalizeFrom3Sigma(){
  for(int b=0;b<_numBeams;b++){
    for(int s=0;s<_slicesPerSpot;s++){
      for(int a=0;a<_numAzimuths;a++){
	if(_numSamples[b][s][a] < _minNumSamples){
...
// KprTable::Write                //
//--------------------------------//   
int KprTable::Write(const char* filename){
      if (filename == NULL)
      return(0);
...
//----------------------------------//
// KprTable::_WriteHeader           //
//----------------------------------//

int 
KprTable::_WriteHeader(FILE* ofp)
{
...
//----------------------------------//
// KprTable::_ReadHeader           //
//----------------------------------//

int 
KprTable::_ReadHeader(FILE* ifp)
{
...
//----------------------------------//
// KprTable::_WriteTable           //
//----------------------------------//

int 
KprTable::_WriteTable(FILE* ofp)
{
  for(int b=0;b<_numBeams;b++){
    for(int s=0;s<_slicesPerSpot;s++){
