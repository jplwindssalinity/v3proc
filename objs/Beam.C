...
Beam::SetBeamPattern(
    ...
    double   x_spacing,
    double   y_spacing,
    double   electrical_boresight_Em,
    double   electrical_boresight_Am,
    float**  power_gain)
{
    //------------------------//
    ...
    float*  gain)
{
        ...
        (iy1 > _Ny - 2))
    {
        ...
        {
            fprintf(stderr,
                "GetPowerGain: requested point out of range (%g, %g)\n",
     ...
     const Beam& from)
{
    ...
    if (_power_gain != NULL)
        free_array((void*)_power_gain, 2, _Nx, _Ny);
    ...
    if (from._power_gain == NULL)
    {
        _power_gain = from._power_gain;
    }
    else
    {
        _power_gain = (float**)make_array(sizeof(float), 2, _Nx, _Ny);
        ...
        {
            fprintf(stderr,
                "Beam::operator=:Can't allocate a pattern array\n");
            exit(1);
        }
        for (int i = 0; i < _Nx; i++)
        {
