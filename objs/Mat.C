    ...
    const Mat&  matrix)
{
    ...
    //------------------------------//
    // zero out small vector values //
    //------------------------------//

    for (int i = 0; i < w._mSize; i++) {
        if (fabs(w._v[i]) < 1e-06)
            w._v[i] = 0.0;
        else
            w._v[i] = 1.0 / w._v[i];
    }
    ...
    const Mat&  matrix)
{
    ...
    if (! _Allocate(matrix._nSize, matrix._mSize)) {
        return(0);
    }
    ...
    const Mat&  b)
{
    ...
    if (! _Allocate(a._mSize, b._nSize)) {
        return(0);
    }
    ...
    for (int i = 0; i < _mSize; i++) {
        for (int j = 0; j < _nSize; j++) {
            _m[i][j] = 0.0;
            for (int k = 0; k < a._nSize; k++) {
                _m[i][j] += am[i][k] * bm[k][j];
            }
        }
    }
    return(1);
...
Mat::_Allocate(
    const Mat&  a)
{
    return(_Allocate(a._mSize, a._nSize));
}
    ...
    int  n_size)
{
    ...
    //---------------------------------------//
    // allocate the column array of pointers //
    //---------------------------------------//
 
    _m = (double **)malloc(m_size * sizeof(double *));
    if (_m == NULL)
        return(0);
...
// Mat::Inverse //
//--------------//
// Sets this Mat to contain the inverse of a
