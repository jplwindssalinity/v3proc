...
#define ANC_TMP2M_FILE_KEYWORD "ANC_TMP2M_FILE"
...
int main(int argc, char* argv[]) {
    ...
    if(argc == 5) {
        ...
        ncep_adj_file = argv[10];
    }

    NCEP_ADJ ncep_adj(ncep_adj_file);
    ...
    char* anc_tmp2m_file = config_list.Get(ANC_TMP2M_FILE_KEYWORD);
    ...
    CAP_ANC_L2B cap_anc_tmp2m(anc_tmp2m_file);
    ...
    std::vector<float> anc_spd(l2b_size), anc_dir(l2b_size), anc_spd_adj(l2b_size);
    std::vector<float> anc_sss(l2b_size), anc_sst(l2b_size), anc_swh(l2b_size);
    std::vector<float> smap_sss(l2b_size), smap_spd(l2b_size);
    std::vector<float> smap_sss_adj(l2b_size), smap_spd_adj(l2b_size);
    ...
    for(int ati=0; ati<nati; ++ati) {
        ...
        for(int cti=0; cti<ncti; ++cti) {
            ...
            smap_sss_adj[l2bidx] = FILL_VALUE;
            smap_spd[l2bidx] = FILL_VALUE;
            smap_spd_adj[l2bidx] = FILL_VALUE;
            n_h_fore[l2bidx] = 0;
            n_h_aft[l2bidx] = 0;
            n_v_fore[l2bidx] = 0;
            n_v_aft[l2bidx] = 0;
            ...
            float this_dt = 
                cap_anc_tmp2m.data[0][anc_ati][anc_cti] - anc_sst[l2bidx];

            anc_spd_adj[l2bidx] = ncep_adj.Get(anc_spd[l2bidx], this_dt);
            ...
            if(tb_ml_avg.NodeCount() > 0) {
                ...
                // Combined SSS and SPD
                anc_spd_std_prior = 1.5;
                cap_gmf.Retrieve(
                    &tb_ml_avg, NULL, anc_spd_adj[l2bidx], anc_dir[l2bidx],
                    anc_sss[l2bidx], anc_spd_adj[l2bidx], anc_dir[l2bidx],
                    anc_sst[l2bidx], anc_swh[l2bidx], 0, anc_spd_std_prior, 0,
                    1, CAPGMF::RETRIEVE_SPEED_SALINITY,
                    &final_spd, &final_dir, &final_sss, &final_obj);

                smap_sss_adj[l2bidx] = final_sss;
                smap_spd_adj[l2bidx] = final_spd;

                anc_spd_std_prior = 100;
                cap_gmf.Retrieve(
                    &tb_ml_avg, NULL, anc_spd_adj[l2bidx], anc_dir[l2bidx],
                    smap_sss[l2bidx], anc_spd_adj[l2bidx], anc_dir[l2bidx],
                    anc_sst[l2bidx], anc_swh[l2bidx], 0, anc_spd_std_prior, 0,
                ...
                if((cnts_fore > 0 && fabs(inc_fore[l2bidx]-40) > 0.2) ||
                   (cnts_aft > 0 && fabs(inc_aft[l2bidx]-40) > 0.2))
    ...
    H5LTmake_dataset(
        file_id, "anc_spd_adj", 2, dims, H5T_NATIVE_FLOAT, &anc_spd_adj[0]);
    ...
    H5LTmake_dataset(
        file_id, "smap_spd_adj", 2, dims, H5T_NATIVE_FLOAT, &smap_spd_adj[0]);
    H5LTmake_dataset(
        file_id, "smap_sss_adj", 2, dims, H5T_NATIVE_FLOAT, &smap_sss_adj[0]);
