...
//
// SYNOPSIS
//    wr_dir <ins_config_file> <hdf_l2a_file> <hdf_l2b_file>
//        <output_dir_file>
//
// DESCRIPTION
//    Calculates the retrieved direction relative to the along track
//    direction.
//
// OPTIONS
//
...
//    <output_dir_file>  The output file.
//
// EXAMPLES
//    An example of a command line is:
//      % wr_dir $cfg l2a.203 l2b.203 203.dir
//
// ENVIRONMENT
...
const char* usage_array[] = { "<ins_config_file>", "<hdf_l2a_file>",
    "<hdf_l2b_file>", "<output_dir_file>", 0 };
    ...
    char*  argv[])
{
    ...
    const char* output_dir_file = argv[optind++];
    ...
    FILE* ofp = fopen(output_dir_file, "w");
    if (ofp == NULL)
    {
    ...
    //--------------------------//
    // initialize the dir array //
    //--------------------------//

    unsigned char dir_array[1624][76];
    for (int i = 0; i < 1624; i++)
    {
        for (int j = 0; j < 76; j++)
        {
            dir_array[i][j] = 255;    // flag as no data
        }
    }
...
//            wvc.FreeContents();
            ...
            if (dir_val > pi_over_two)
                dir_val = pi - dir_val;
            dir_val = pi_over_two - dir_val;    // make it rel to at
            dir_val *= rtd;
            ...
            dir_array[target_idx][cell_idx] = (char)(dir_val / DIR_SCALE + 0.5);
        }
        for (int cell_idx = 0; cell_idx < MAX_CROSS_TRACK; cell_idx++)
        {
            MeasList* meas_list = &(meas_list_row[cell_idx]);
    ...
    fwrite(dir_array, sizeof(char), 76 * 1624, ofp);
    fclose(ofp);
