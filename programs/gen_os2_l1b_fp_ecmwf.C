    ...
    char*    argv[])
{
  ...
  char scan_start_time[num_l1b_frames][22];
  
  vector< vector<int16>  > fp_lat_old(2);
  vector< vector<uint16> > fp_lat_new(2);
  vector< vector<uint16> > fp_lon(2);
  vector< vector<float> > espd(2);
  vector< vector<float> > edir(2);
  vector< vector<char>  > island(2);  
  
  for( int i_pol = 0; i_pol < 2; ++i_pol ) {
    int n_scans = ( i_pol == 0 ) ? 281 : 282;
    fp_lat_old[i_pol].resize( num_l1b_frames * n_scans );
    fp_lat_new[i_pol].resize( num_l1b_frames * n_scans );
    fp_lon[i_pol].resize( num_l1b_frames * n_scans );
    espd[i_pol].resize(   num_l1b_frames * n_scans );
    edir[i_pol].resize(   num_l1b_frames * n_scans );
    island[i_pol].resize( num_l1b_frames * n_scans );    
  }
  
  read_SDS_h5( g_id, "Scan_start_time",                &scan_start_time[0]);  
  read_SDS_h5( g_id, "Inner_beam_footprint_longitude", &fp_lon[0][0] );
  read_SDS_h5( g_id, "Outer_beam_footprint_longitude", &fp_lon[1][0] );
  
  if( use_new_scale_factors ) {
    read_SDS_h5( g_id, "Inner_beam_footprint_latitude",  &fp_lat_new[0][0] );  
    read_SDS_h5( g_id, "Outer_beam_footprint_latitude",  &fp_lat_new[1][0] );  
  } else {
    read_SDS_h5( g_id, "Inner_beam_footprint_latitude",  &fp_lat_old[0][0] );  
    read_SDS_h5( g_id, "Outer_beam_footprint_latitude",  &fp_lat_old[1][0] );
  }
  ...
  for( int i_frame = 0; i_frame < num_l1b_frames; ++i_frame ) {
    ...
    char frame_time_str[22];
    
    strcpy( frame_time_str, &scan_start_time[i_frame][0] );
    ...
    for( int i_pol = 0; i_pol < 2; ++i_pol ) {
      ...
      for( int i_scan = 0; i_scan < n_scans; ++i_scan ) {
        ...
        double wvc_lat_deg, wvc_lon_deg;
        
        if( use_new_scale_factors ) {
          wvc_lat_deg = -90+0.002757*double(fp_lat_new[i_pol][scan_ind]);
          wvc_lon_deg =   0+0.005515*double(fp_lon[i_pol][scan_ind]);        
        } else {
          wvc_lat_deg = 0.01*double(fp_lat_old[i_pol][scan_ind]);
          wvc_lon_deg = 0.01*double(fp_lon[i_pol][scan_ind]);
        }
        
        if( wvc_lon_deg < 0    ) wvc_lon_deg += 360;
        if( wvc_lon_deg >= 360 ) wvc_lon_deg -= 360;
        
        if( use_new_scale_factors & fp_lon[i_pol][scan_ind] == 65535 ) {
          espd[i_pol][scan_ind] = -1;
          edir[i_pol][scan_ind] =  0;
          continue;
        }        
        ...
        island[i_pol][scan_ind] = 0;
        if( landmap_file && qs_landmap.IsLand(lon_rad,lat_rad,2) )
          island[i_pol][scan_ind] = 1;
        ...
        espd[i_pol][scan_ind] = sqrt( uuu_ * uuu_ + vvv_ * vvv_ );
        edir[i_pol][scan_ind] = RAD_TO_DEG( atan2( uuu_, vvv_ ) );
      }
    }
  }
  ...
  int n_scans_inner = 281;
  int n_scans_outer = 282;
  
  fwrite( &num_l1b_frames, sizeof(int),                    1, ofp );
  fwrite( &n_scans_inner,  sizeof(int),                    1, ofp );
  fwrite( &n_scans_outer,  sizeof(int),                    1, ofp );
  fwrite( &espd[0][0],     sizeof(float), 281*num_l1b_frames, ofp );
  fwrite( &edir[0][0],     sizeof(float), 281*num_l1b_frames, ofp );
  fwrite( &espd[1][0],     sizeof(float), 282*num_l1b_frames, ofp );
  fwrite( &edir[1][0],     sizeof(float), 282*num_l1b_frames, ofp );

  if( landmap_file ) {
    fwrite( &island[0][0], sizeof(char), 281*num_l1b_frames, ofp );  
    fwrite( &island[1][0], sizeof(char), 282*num_l1b_frames, ofp );  
  }
  // Close L1B file
  H5Gclose(g_id);
  H5Fclose(h_id);
