    ...
    char*  argv[])
{
   ...
   while(gnf.ReadLine()){
	...
	float*  mlpinvec = (float*) malloc(sizeof(float)*inMLP-1);
...
//#endif
	  ...
	  rdist.rainContaminateSigma0(j,mlpinvec);
        ...
        // copy speed net inputs to dir net input array
        for(int c=0;c<inMLP-1;c++) mlpinvec_dir[c]=mlpinvec[c];
  
        // append estimated speed to dir net input array
        MLP* m=spdarr.getMLP(0,gnf.ctd);
        float dummyout=0;
        m->Forward(&dummyout,mlpinvec);
	mlpinvec_dir[inMLP-1]=m->outp[0];
