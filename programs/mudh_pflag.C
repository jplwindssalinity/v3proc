...
//
// SYNOPSIS
//    mudh_pflag [ -t threshold ] <ins_config_file> <mudhtab_file>
//        <hdf_l2a_file> <hdf_l2b_file> <output_pflag_file>
//
// DESCRIPTION
//    Performs Pr classification and writes out two arrays:
//      A floating point Pr array and a byte flag array.
...
//      % mudh_pflag $cfg mudh.tab l2a.203 l2b.203 203.pflag
//
// ENVIRONMENT
...
const char* usage_array[] = { "[ -t threshold ]", "<ins_config_file>",
    "<mudhtab_file>", "<hdf_l2a_file>", "<hdf_l2b_file>",
    "<output_pflag_file>", 0 };
    ...
    char*  argv[])
{
    ...
    for (int idx = 0; idx < l2a_length; idx++)
    {
        ...
        for (int cti = 0; cti < CT_WIDTH; cti++)
        {
            ...
            //----------------------------------------------//
            // look up the value from the probability table //
            //----------------------------------------------//
            ...
            float pr_value = (float)rain_tab[inbd][ispd][idir][imle];
            index_tab[ati][cti] = pr_value;

            if (pr_value < 0.0 || pr_value > 1.0)
            {
                // this doesn't seem like a probability
                index_tab[ati][cti] = -1.0;
                flag_tab[ati][cti] = 2;
                continue;
            }
