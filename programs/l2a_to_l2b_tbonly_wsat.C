...
#define ANC_WSAT_FILE_KEYWORD "ANC_WSAT_FILE"
...
#define QUAL_FLAG_WSAT_AVAILABLE 0x200
...
int main(int argc, char* argv[]) {
    ...
    char* anc_wsat_file = config_list.Get(ANC_WSAT_FILE_KEYWORD);
    config_list.ExitForMissingKeywords();
    ...
    CAP_ANC_L2B cap_anc_wsat;

    if(anc_wsat_file) cap_anc_wsat.Read(anc_wsat_file);
    ...
    for(int ati=0; ati<nati; ++ati) {
        ...
        for(int cti=0; cti<ncti; ++cti) {
            ...
            if(anc_wsat_file) {
                float wsat_spd = cap_anc_wsat.data[0][anc_ati][anc_cti];
                float wsat_dir = cap_anc_wsat.data[1][anc_ati][anc_cti];
                float wsat_rain = cap_anc_wsat.data[2][anc_ati][anc_cti];
                float wsat_dt = cap_anc_wsat.data[3][anc_ati][anc_cti];

                if(wsat_spd > 0 & wsat_dt < 100 & wsat_rain < 0.01) {
                    anc_spd[l2bidx] = wsat_spd;
            ...
            if(tb_ml_avg.NodeCount() > 0) {
                ...
                // Check for missing windsat / ssmi matchups
                if(missing_wsat)
                    quality_flag[l2bidx] |= QUAL_FLAG_WSAT_AVAILABLE;
                ...
                uint16 QUAL_MASK = (
                    ...
                    QUAL_FLAG_POINTING | QUAL_FLAG_WSAT_AVAILABLE);
