...
float GetTrueObj(float direrr, WindVectorPlus* wvp, MeasList* ml,GMF* gmf, Kp* kp){
  ...
  float phierr=wvp->dir+direrr;
  while(phierr>=2*pi) phierr-=2*pi;
  while(phierr<0) phierr+=2*pi;
  float obj=gmf->_ObjectiveFunction(ml,wvp->spd,phierr,kp);
  return(obj);
} 
    ...
    char*  argv[])
{
    ...
    // set up random direction error model for use in traing objarr
    ...
    MLPDataArray objarr(out_obj_file,net_obj_file,MLPdim1length,MLPdim1min,MLPdim1max,MLPdim2length,MLPdim2min,MLPdim2max,inMLP,outMLP,n*CTS,"bestobjnet",hn); 
    spdarr.nepochs=2000;
   ...
   for(int d=0;d<ndatasets;d++){
	...
	spdarr.addSample(reldir,ctd,mlpinvec,&(twvp.spd));
        float rand_direrr=randdirgen.GetNumber();
	float obj=GetTrueObj(rand_direrr,&twvp,meas_list,&gmf,&kp);
        float perfobj=0.5; // since reldir is expected value prob the true valueis greater or lesser is 0.5
	//objarr.addSample(reldir,ctd,mlpinvec,&perfobj); // add an perfect dir sample
        float truereldir=reldir;
	...
	if(USE_CTD_INPUT==2){
	  ...
	  mlpinvec[inMLP-2]=reldir;
        ...
        if(reldir>truereldir) obj=1;
	else obj=0;
	objarr.addSample(reldir,ctd,mlpinvec,&obj);// add an erroneous one
