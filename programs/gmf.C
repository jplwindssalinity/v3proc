		...
		for (int pol = 0; pol < POLS; pol++)
		{
			for (int inc = 16; inc < INCS; inc += INC_STEP)
			{
				...
				for (int spd = 1; spd < SPDS; spd += 1)
				{
					...
					gmf.GetCoefs(pol, dinc, dspd, &(a0[pol][inc][spd]),
						&(a1[pol][inc][spd]), &(a2[pol][inc][spd]));
					a0[pol][inc][spd] = 10.0 * log10(a0[pol][inc][spd]);
					a1[pol][inc][spd] = 10.0 * log10(a1[pol][inc][spd]);
					a2[pol][inc][spd] = 10.0 * log10(a2[pol][inc][spd]);
				}
			}
		}
	}
