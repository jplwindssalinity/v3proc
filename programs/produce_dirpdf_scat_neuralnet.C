...
const char* usage_array[] = { "<sim_config_file>", "<max_speed>", "<rainfilelist>""<geom_input_file>","<dataset_output_file>","<num_samples>","[num_hidno]", "[max_epochs]", "[momentum]","[stepsize]","[max_bad_epochs]","[min_speed]",0};
    ...
    char*  argv[])
{
    ...
    *****/
    ...
    MLPDataArray dirarr(out_dirpdf_file,net_dirpdf_file,MLPdim1length,MLPdim1min,MLPdim1max,MLPdim2length,MLPdim2min,MLPdim2max,inMLP,outMLP,n*CTS,"dirpdfnet",hn); 
   ...
   while(gnf.ReadLine()){
	...
	// Create Measurement List
        //-------------------------
	float*  mlpinvec = (float*) malloc(sizeof(float)*inMLP);
...
//#endif
	...
	dirarr.addSampleInOrderAndWrite(0,gnf.ctd,mlpinvec,&reldir);
        free(mlpinvec);
