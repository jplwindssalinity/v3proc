...
//
// SYNOPSIS
//    s0ave <ins_config_file> <hdf_l2a_file> <output_s0_file>
//
// DESCRIPTION
...
//    <output_s0_file>  The output file.
//
// EXAMPLES
//    An example of a command line is:
//      % mudh_calc $cfg l2a.203  203.s0
//
// ENVIRONMENT
...
float s0_h_array[AT_WIDTH][CT_WIDTH];
float s0_v_array[AT_WIDTH][CT_WIDTH];
char num_h_array[AT_WIDTH][CT_WIDTH];
char num_v_array[AT_WIDTH][CT_WIDTH];
    ...
    char*  argv[])
{
    ...
    const char* output_s0_file = argv[optind++];
    ...
    FILE* ofp = fopen(output_s0_file, "w");
    if (ofp == NULL)
    {
...
//            int land_flag = surface_flag[j] & 0x00000001;
        ...
        for (int cti = 0; cti < CT_WIDTH; cti++)
        {
                ...
                meas = meas_list->GetNext())
            {
                ...
                {
                    num_v_array[ati][cti]++;
                    s0_v_array[ati][cti]+=meas->value;
                }
		else{
                    num_h_array[ati][cti]++;
                    s0_h_array[ati][cti]+=meas->value;
		}
            }
	    if(num_v_array[ati][cti]>0)
	      s0_v_array[ati][cti]/=num_v_array[ati][cti];
	    if(num_h_array[ati][cti]>0)
	      s0_h_array[ati][cti]/=num_h_array[ati][cti];
        }
        for (int cti = 0; cti < CT_WIDTH; cti++)
        {
            MeasList* meas_list = &(meas_list_row[cti]);
    ...
    fwrite(s0_h_array, sizeof(float), CT_WIDTH * AT_WIDTH, ofp);
    fwrite(s0_v_array, sizeof(float), CT_WIDTH * AT_WIDTH, ofp);
    fwrite(num_h_array, sizeof(char), CT_WIDTH * AT_WIDTH, ofp);
    fwrite(num_v_array, sizeof(char), CT_WIDTH * AT_WIDTH, ofp);
    fclose(ofp);
