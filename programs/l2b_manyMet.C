    ...
    char*    argv[])
{
    ...
    sprintf(filename, "%s.%s.%s", output_base, "sel_rms_spd_err", "v");
    FILE* ofp = fopen(filename, "w");

    fprintf(ofp, "%i\n", cross_track_bins);
        for (int ii = 0; ii < cross_track_bins; ++ii)
      {
        fprintf(ofp, "%i %g\n", count_array[ii], value_array[ii]);
      }
    ...
    sprintf(filename, "%s.%s.%s", output_base, "sel_spd_bias", "v");
    ofp = fopen(filename, "w");

    fprintf(ofp, "%i\n", cross_track_bins);
        for (int ii = 0; ii < cross_track_bins; ++ii)
      {
        fprintf(ofp, "%i %g\n", count_array[ii], value_2_array[ii]);
      }
    ...
    sprintf(filename, "%s.%s.%s", output_base, "sel_rms_dir_err", "v");
    ...
    sprintf(filename, "%s.%s.%s", output_base, "sel_dir_bias", "v");
    ...
    sprintf(filename, "%s.%s.%s", output_base, "skill", "v");
    ...
    sprintf(filename, "%s.%s.%s", output_base, "within", "v");
    ...
    sprintf(filename, "%s.%s.%s", output_base, "vector_correlation", "v");
    ...
    sprintf(filename, "%s.%s.%s", output_base, "avg_nambig", "v");
    ...
    sprintf(filename, "%s.%s.%s", output_base, "dir_den", "v");

    ofp = fopen(filename, "w");
    fprintf(ofp, "%i\n", cross_track_bins);
        for (int ii = 0; ii < cross_track_bins; ++ii)
      {
        fprintf(ofp, "%i %g\n", count_array[ii], float(uint_array[ii]));
      }
    ...
    sprintf(filename, "%s.%s.%s", output_base, "near_rms_spd_err", "v");
    ...
    sprintf(filename, "%s.%s.%s", output_base, "near_spd_bias", "v");
    ...
    sprintf(filename, "%s.%s.%s", output_base, "near_rms_dir_err", "v");
    ...
    sprintf(filename, "%s.%s.%s", output_base, "near_dir_bias", "v");
