...
//
// SYNOPSIS
//    get_foff [ -f fit_base ] <config_file> <foff_file>
//      <echo_file...>
//
// DESCRIPTION
...
//      <foff_file>     The output foff file.
//      <echo_file...>  The echo data files.
//
// EXAMPLES
//    An example of a command line is:
//      % get_foff -f fit qscat.cfg foff echo1.dat echo2.dat
//
// ENVIRONMENT
...
int     process_orbit_step(int beam_idx, int orbit_step,
            const char* fit_base);
int     accumulate(int beam_idx, double azimuth, double foff);
...
const char* usage_array[] = { "[ -f fit_base ]", "<config_file>",
    "<foff_file>", "<echo_file...>", 0 };
    ...
    char*  argv[])
{
    ...
    g_foff = (double **)make_array(sizeof(double), 2,
        NUMBER_OF_QSCAT_BEAMS, max_count);
    if (g_foff == NULL)
    {
        fprintf(stderr,
            "%s: error allocating foff array (%d x %d)\n", command,
            NUMBER_OF_QSCAT_BEAMS, max_count);
        exit(1);
    }
        ...
        target_orbit_step++)
    {
        ...
        for (int file_idx = start_idx; file_idx < end_idx; file_idx++)
        {
            ...
            do
            {
                ...
                //---------------------------------//
                // set CDS for orbit fraction calc //
                //---------------------------------//

                qscat.cds.orbitTime = echo_info.orbitTicks;
                double orbit_position = qscat.cds.OrbitFraction();
                ...
                for (int spot_idx = 0; spot_idx < spots_per_frame; spot_idx++)
                {
                    ...
                    //--------------------------------------//
                    // determine the right value of delta f //
                    //--------------------------------------//
                    // the delta f that makes the peak specs line up

                    float delta_f_1 = -20000.0;
                    float fbb_1 = fbb_table.GetFbb(beam_idx, azimuth,
                        orbit_position, delta_f_1);
                    float delta_f_2 = 20000.0;
                    float fbb_2 = fbb_table.GetFbb(beam_idx, azimuth,
                        orbit_position, delta_f_2);
                    while (delta_f_2 - delta_f_1 > 1.0)
                    {
                        float delta_f_mid = (delta_f_1 + delta_f_2) / 2.0;
                        float fbb_mid = fbb_table.GetFbb(beam_idx, azimuth,
                            orbit_position, delta_f_mid);
                        float fbb_dif = fbb_mid -
                            echo_info.measSpecPeakFreq[spot_idx];
                        if (fbb_dif > 0.0)
                        {
                            delta_f_2 = delta_f_mid;
                            fbb_2 = fbb_mid;
                        }
                        else
                        {
                            delta_f_1 = delta_f_mid;
                            fbb_1 = fbb_mid;
                        }
                    }

                    float use_delta_f = (delta_f_1 + delta_f_2) / 2.0;
                    float foff = use_delta_f - echo_info.deltaF[spot_idx];

                    accumulate(beam_idx, azimuth, foff);
                }
    ...
    char filename[1024];
/*
    sprintf(filename, "%s.raw", foff_base);
...
*/
    ...
    sprintf(filename, "%s.maxoff", foff_file);
    ...
    const char*  fit_base)
{
    ...
    double a, p, c;
    sinfit(g_azimuth[beam_idx], g_foff[beam_idx], NULL,
    ...
    if (fit_base)
    {
        ...
        fprintf(ofp, "@ yaxis label %cF-off (kHz)%c\n", QUOTE,
            QUOTE);
        for (int i = 0; i < g_count[beam_idx]; i++)
        {
            fprintf(ofp, "%g %g\n", g_azimuth[beam_idx][i] * rtd,
                g_foff[beam_idx][i] / 1000.0);
        }
        fprintf(ofp, "&\n");
